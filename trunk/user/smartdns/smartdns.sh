...
Check_ss() {
    if [ $(nvram get ss_enable) = "1" ] && [ $(nvram get ss_mode) = "2" ] && [ "$ss_router_proxy" = "4" ]; then
        logger -t "SmartDNS" "系统检测到SS模式为绕过大陆模式，并且启用了pdnsd,请先调整SS解析使用SmartDNS+手动配置模式！程序将退出。"
        nvram set sdns_enable=0
        exit 0
    else
        logger -t "SmartDNS" "检测端口没有被占用，准备开始程序。"
    fi
}

smartdns_folder() {
    if [ ! -d "$storage_Path/smartdns" ] ; then
        tar zxf "/etc_ro/smartdns.tar.gz" -C "$storage_Path" && sleep 2
    fi
}

smartdns_rule() {
    if [ ! -f "$smartdns_address_Conf" ] || [ ! -s "$smartdns_address_Conf" ] ; then
        cp -f "/$storage_Path/smartdns/smartdns_address.conf" $smartdns_address_Conf && \
        chmod 644 "$smartdns_address_Conf"
    fi
    if [ ! -f "$smartdns_blacklist_Conf" ] || [ ! -s "$smartdns_blacklist_Conf" ] ; then
        cp -f "/$storage_Path/smartdns/smartdns_blacklist-ip.conf" $smartdns_blacklist_Conf && \
        chmod 644 "$smartdns_blacklist_Conf"
    fi
    if [ ! -f "$smartdns_whitelist_Conf" ] || [ ! -s "$smartdns_whitelist_Conf" ] ; then
        cp -f "/$storage_Path/smartdns/smartdns_whitelist-ip.conf" $smartdns_whitelist_Conf && \
        chmod 644 "$smartdns_whitelist_Conf"
    fi
    if [ ! -f "$smartdns_custom_Conf" ] || [ ! -s "$smartdns_custom_Conf" ] ; then
        cp -f "$storage_Path/smartdns/smartdns_custom_Conf" $smartdns_custom_Conf && \
        chmod 644 "$smartdns_custom_Conf"
    fi
}
...
func_chnroute_file() {
    dir_chnroute_file="$storage_Path/chinadns/chnroute.txt"
    if [ ! -f "$dir_chnroute_file" ] || [ ! -s "$dir_chnroute_file" ]; then
        [ ! -d $storage_Path/chinadns ] && mkdir -p "$storage_Path/chinadns"
        tar jxf "/etc_ro/chnroute.bz2" -C "$storage_Path/chinadns"
...
Change_dnsmasq () {
    ...
    stop)
        ...
        then
            sed -i '/no-resolv/d; /server=127.0.0.1/d; /port=0/d' $dnsmasq_Conf
        fi
...
Start_smartdns () {
    ...
    #mv -f "$smartdns_tmp_Conf" $smartdns_custom_Conf
    awk '!x[$0]++' "$smartdns_tmp_Conf" > $smartdns_custom_Conf
...
Stop_smartdns () {
    ...
    Change_dnsmasq
    Change_iptable
    iptables-save -c | grep -v "smartdns" | iptables-restore -c
    for setname in $(ipset -n list | grep "smartdns"); do
