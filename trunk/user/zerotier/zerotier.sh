...
start_instance() {
    ...
    if [ -z "$secret" ]; then
        logger -t "zerotier" "The device key is empty. generating a key. Please wait..."
...
creat_moon(){
    ...
    logger -t "zerotier" "Build a Moon transit server for ZeroTier and generate Moon configuration"
    if [ -z "$moonip" ]; then
        #wanip
    ...
    if [ -e $config_path/identity.public ]; then
        ...
        if 'sed -i "s/\[\]/\[ \"$ip_addr\/9993\" \]/" $config_path/moon.json >/dev/null 2>/dev/null'; then
            logger -t "zerotier" "Successfully generated moon configuration"
        else
            logger -t "zerotier" "Failed to generate moon configuration"
        fi
        logger -t "zerotier" "Generate signature"
        cd $config_path
        pwd
        $PROGIDT genmoon $config_path/moon.json
        [ $? -ne 0 ] && return 1
        logger -t "zerotier" "moons.d"
        if [ ! -d "$config_path/moons.d" ]; then
            mkdir -p $config_path/moons.d
        fi
        #moon server
        mv $config_path/*.moon $config_path/moons.d/ >/dev/null 2>&1
        logger -t "zerotier" "moonNode creation completed"
